...
def _average(db, bank_id, code, report_date):
    """Port of trades_done_average: '=cost_to_date/balance' formula, else running-average fallback."""
    rows = get_transactions(db, report_date.isoformat(), code, bank_id)
...
def position_records(db, bank_ref, bank_id, ccy_id, report_date: date) -> dict:
    ...
    held at `bank_ref` with a non-zero long-leg share balance as of `report_date`.
    """
    bank_row = db.execute(
    ...
    cutoff = report_date if indicative == 'YES' else wd.previous_day(report_date)
    ...
    rows = db.execute(
        ...
        (bank_ref, ccy_id, report_date.isoformat())
    ).fetchall()
    ...
    for r in rows:
        ...
        result[code] = {
            ...
            'average': _average(db, bank_id, code, report_date),
