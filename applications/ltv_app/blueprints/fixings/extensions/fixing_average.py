class FixingAverage:
    def __init__(self, db, trade_date, code, bank_id, accus):
        self.transactions = get_transactions(db, trade_date, code, bank_id)

        self.accus = accus

    def average(self):
        return get_average(self.transactions, self.accus)
...
def get_average(transactions, accus):
    balance = 0
    cost_to_date = 0.0
    average = 0
    last_average = 0

    for batch in (transactions, accus):
        for row in batch:
